...
const Pricelists = () => {
  ...
  const handleSavePricelist = async (pricelist) => {
    if (selectedPricelist && selectedPricelist.name) {
      // Update pricelist
      try {
        await api.put(`/pricelists/${selectedPricelist.name}`, pricelist, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        });
  ...
  const handleSaveProductLine = async (productLine) => {
    if (selectedProductLine && selectedProductLine.id) {
      ...
      try {
        await api.put(
          `/pricelist_line/${selectedProductLine.id}`,
          productLine,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      // Add product line
      try {
        await api.post("/pricelist_line", productLine, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        });
  ...
  const handleConfirmDeleteProductLine = async () => {
    try {
      await api.delete(`/pricelist_line/${selectedProductLine.id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
